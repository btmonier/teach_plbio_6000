...
for epoch in range(epochs):
  ...
  for batch_index_start in range(0, len(training_set), batch_size):
    ...
    training_inputs = torch.tensor([sample['1hot_seq'] for sample in batch], dtype=torch.float32)
    training_targets = torch.tensor([sample['bound'] for sample in batch], dtype=torch.float32).reshape(-1, 1)
